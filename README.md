...
> **Chrome/Edge extension ID stability:** without a fixed `"key"` in `manifest.json`, Chrome and Edge each assign a different extension ID per install context (unpacked vs. each store's published copy, or even each machine an unpacked copy is loaded on), which changes the `chromiumapp.org` redirect URI and breaks sign-in — the same issue this project hit once already when the OAuth client was tied to a since-changed ID. `manifest.json` now pins a `"key"` (the public half of `extension_key.pem`, generated locally and **not** committed — see `.gitignore`), so the extension ID is deterministic: `ahgdmcnjnlnjfpjknpkfmdhaodjmlhmh`, i.e. redirect URI `https://ahgdmcnjnlnjfpjknpkfmdhaodjmlhmh.chromiumapp.org/`. That ID stays identical across every machine that loads this repo unpacked and (once published) across the Chrome Web Store install for every end user — register it once in the OAuth client's **Authorized redirect URIs** and it never needs re-adding. Keep `extension_key.pem` private; if it's ever lost, generate a new one and the ID (and required redirect URI) will change again.
